...
var OllamaAIProvider = class {
  ...
  static async getModels(providerConfig) {
    const { json } = await (0, import_obsidian.requestUrl)({
...
var OpenAICompatibleAIProvider = class {
  ...
  static async getModels(providerConfig) {
    ...
    return json.data.reduce((acc, el) => {
      const name = el.id;
      acc[name] = name;
      return acc;
    }, {});
...
// src/LocalGPTSettingTab.ts
var SEPARATOR = "\u2702\uFE0F";
var LocalGPTSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setHeading().setName("Selected AI provider").addDropdown(
    ...
    new import_obsidian3.Setting(containerEl).setName("Creativity").setDesc("").addDropdown((dropdown) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Use fallback").addToggle((component) => {
    ...
    if (this.useFallback) {
      new import_obsidian3.Setting(containerEl).setName("Fallback AI provider").setDesc(
    ...
    const aiProvider = new import_obsidian3.Setting(containerEl).setHeading().setName("Configure AI provider").setDesc("").addDropdown(
    ...
    if (selectedProviderConfig.type === "ollama" /* OLLAMA */) {
      new import_obsidian3.Setting(containerEl).setName("Ollama URL").setDesc("Default is http://localhost:11434").addText(
      ...
      const ollamaDefaultModel = new import_obsidian3.Setting(containerEl).setName("Default model").setDesc("Name of the default Ollama model to use in prompts");
    ...
    if (selectedProviderConfig.type === "openaiCompatible" /* OPENAI_COMPATIBLE */) {
      const openAICompatible = new import_obsidian3.Setting(containerEl).setName("OpenAI compatible server URL").setDesc("").addText(
      ...
      openAICompatible.descEl.innerHTML = `
			...
			`;
      const apiKey = new import_obsidian3.Setting(containerEl).setName("API key").setDesc("").addText(
      ...
      apiKey.descEl.innerHTML = `
			...
			`;
      const openaiDefaultModel = new import_obsidian3.Setting(containerEl).setName("Default model").setDesc(
    ...
    if (!this.editEnabled) {
      const quickAdd = new import_obsidian3.Setting(containerEl).setName("Quick add").setDesc("").addText((text) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("Add new manually").addButton(
      ...
      if (this.plugin.settings.providers[this.plugin.settings.defaults.provider].type === "ollama" /* OLLAMA */) {
        new import_obsidian3.Setting(containerEl).setName("Model").setDesc("Optional").addDropdown((dropdown) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("Action name").addText((text) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("System prompt").setDesc("Optional").addTextArea((text) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("Prompt").addTextArea((text) => {
      ...
      new import_obsidian3.Setting(containerEl).setName("Replace selected text").setDesc(
      ...
      const actionButtonsRow = new import_obsidian3.Setting(containerEl).setName("");
...
`);
      if (!this.changingOrder) {
        const actionRow = new import_obsidian3.Setting(containerEl).setName(action.name).setDesc("").addButton(
        ...
        const actionRow = new import_obsidian3.Setting(containerEl).setName(action.name).setDesc("");
        if (actionIndex > 0) {
          actionRow.addButton(
    ...
    if (this.plugin.settings.actions.length) {
      new import_obsidian3.Setting(containerEl).setName("").addButton((button) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Reset actions").setDesc(
  ...
  async addNewAction(editingAction) {
    ...
    if (alreadyExistingActionIndex >= 0) {
      ...
      new import_obsidian3.Notice(`Rewritten "${editingAction.name}" action`);
    } else {
      this.plugin.settings.actions = [
        editingAction,
        ...this.plugin.settings.actions
      ];
      new import_obsidian3.Notice(`Added "${editingAction.name}" action`);
    }
    await this.plugin.saveSettings();
  }
};
